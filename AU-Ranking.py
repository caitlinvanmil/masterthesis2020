...
p1c2 = p1c[p1c[" confidence"] >= 0.7]


# In[54]:


p2c2 = p2c[p2c[" confidence"] >= 0.7]


# In[55]:


p3c2 = p3c[p3c[" confidence"] >= 0.7]


# In[56]:


p4c2 = p4c[p4c[" confidence"] >= 0.7]


# In[57]:


p5c2 = p5c[p5c[" confidence"] >= 0.7]


# In[58]:


p6c2 = p6c[p6c[" confidence"] >= 0.7]


# In[59]:


p7c2 = p7c[p7c[" confidence"] >= 0.7]


# In[60]:


p8c2 = p8c[p8c[" confidence"] >= 0.7]


# In[61]:


p9c2 = p9c[p9c[" confidence"] >= 0.7]


# In[62]:


p10c2 = p10c[p10c[" confidence"] >= 0.7]


# In[63]:


p11c2 = p11c[p11c[" confidence"] >= 0.7]


# In[64]:


p12c2 = p12c[p12c[" confidence"] >= 0.7]


# In[65]:


p13c2 = p13c[p13c[" confidence"] >= 0.7]


# In[66]:


p14c2 = p14c[p14c[" confidence"] >= 0.7]


# In[67]:


p15c2 = p15c[p15c[" confidence"] >= 0.7]


# In[68]:


p16c2 = p16c[p16c[" confidence"] >= 0.7]


# In[69]:


p17c2 = p17c[p17c[" confidence"] >= 0.7]


# In[70]:


p18c2 = p18c[p18c[" confidence"] >= 0.7]


# In[71]:


p19c2 = p19c[p19c[" confidence"] >= 0.7]


# In[72]:


p20c2 = p20c[p20c[" confidence"] >= 0.7]


# In[73]:


p21c2 = p21c[p21c[" confidence"] >= 0.7]


# In[74]:


p22c2 = p22c[p22c[" confidence"] >= 0.7]


# In[75]:


p23c2 = p23c[p23c[" confidence"] >= 0.7]


# In[76]:


p24c2 = p24c[p24c[" confidence"] >= 0.7]


# In[77]:


p25c2 = p25c[p25c[" confidence"] >= 0.7]
...
X_padded_2d = X_padded.reshape(25, 17*32607)
X_padded_2d.shape
...
# The article by Loquasto & Seborg (2003, June) suggests that the components should explain at least 85% percent of the variance. In this case, this would result in 18 components.
...
sum(expl_var)-expl_var[23]-expl_var[22]-expl_var[21]-expl_var[20]-expl_var[19]-expl_var[18]
...
X_pca = pca.fit_transform(X_padded_2d)
PCA_df = pd.DataFrame(data = X_pca, columns = ['PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8', 'PC9', 'PC10', 'PC11', 'PC12', 'PC13', 'PC14', 'PC15', 'PC16', 'PC17', 'PC18'])
PCA_df = pd.concat([PCA_df, data['y']], axis = 1)
PCA_df['y'] = LabelEncoder().fit_transform(PCA_df['y'])
PCA_df
...
loadings = pd.DataFrame(pca.components_.T, columns=['PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8', 'PC9', 'PC10', 'PC11', 'PC12', 'PC13', 'PC14', 'PC15', 'PC16', 'PC17', 'PC18'])
loadings
...
loading_matrix = pd.DataFrame(loadings, columns=['PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8', 'PC9', 'PC10', 'PC11', 'PC12', 'PC13', 'PC14', 'PC15', 'PC16', 'PC17', 'PC18'])
loading_matrix
...
for i in range(0,17):
    for j in range(0,18):
        sum_AU_r = sum(loading_matrix.iloc[i*32607:i*32607+32607:, j:j+1].values)
        loadings[i,j] = sum_AU_r[0]/32607
